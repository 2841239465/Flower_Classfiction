...
def render_photo_as_page(filename):
    ...
    return result
#
# 实现图片预测的视图函数update，做到能调用infer函数对上传的图像进行分类并保存预测的结果（该功能定义一个新函数render_photo_as_page来实现），
# 并根据展示图像以及图像预测结果的模板文件show.html传入必要的变量，使得show.html页面能正常的渲染。
